 ...
 */ function routeToRegExp(path, opts) {
        ...
        if (opts.ignoreTrailingSlashes) pattern = pattern.replace(/\/+$/, '') + '/*';
        return {
            keys: keys,
            regexp: new RegExp('^' + pattern + '(?:[?#]|$)', opts.caseInsensitiveMatch ? 'i' : '')
        };
    }
    /* global routeToRegExp: false */ /* global shallowCopy: false */ // `isArray` and `isObject` are necessary for `shallowCopy()` (included via `src/shallowCopy.js`).
 ...
 *
 */ /* global -ngRouteModule */ var ngRouteModule = angular.module('ngRoute', []).info({
 ...
 */ function $RouteProvider() {
   ...
   */ this.when = function(path, route) {
            ...
            if (angular.isUndefined(routeCopy.reloadOnUrl)) routeCopy.reloadOnUrl = true;
   ...
   */ isEagerInstantiationEnabled = true;
     ...
     */ var forceReload = false, preparedRoute, preparedRouteIsUpdateOnly, $route = {
           ...
           */ updateParams: function(newParams) {
                        ...
                        } else throw $routeMinErr('norout', 'Tried updating route with no current route');
                    }
                };
    ...
    instantiateRoute.$inject = [
        '$injector'
    ];
    function instantiateRoute($injector) {
        if (isEagerInstantiationEnabled) // Instantiate `$route`
        $injector.get('$route');
    }
                    ...
                    if (angular.isDefined(template)) {
                        ...
                        var clone1 = $transclude(newScope, function(clone) {
                            $animate.enter(clone, null, currentElement || $element).done(function onNgViewEnter(response) {
                                if (response !== false && angular.isDefined(autoScrollExp) && (!autoScrollExp || scope.$eval(autoScrollExp))) $anchorScroll();
                            });
                            cleanupLastView();
                        });
                        currentElement = clone1;
...
//# sourceMappingURL=privacy-seoflow.246775db.js.map
